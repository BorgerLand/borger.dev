...
- Each tick ID may be resimulated multiple times due to [**rollback**](./rollback-and-misprediction.md#rollback). Because of this, the code running in the loop must be [**deterministic**](./determinism.md) depending on the [**trade-off**](./trade-offs.md) used.
...
- Although the presentation loop itself does not undergo [**rollback**](./rollback-and-misprediction.md#rollback), it does have to be aware that the output it's receiving may have undergone rollback. Again, this is achieved by blindly trusting the simulation's output, even when it makes no sense due to [**mispredictions**](./rollback-and-misprediction.md#rollback): players teleporting, entities spawning and immediately despawning, etc.
